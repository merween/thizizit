...
if (isset($_POST['amenities'])) {
    $delete = mysqli_query($connect, "DELETE FROM room_facility where room_id = '$text_id'");
	$type = mysqli_query($connect, "SELECT * FROM type_of_room");
	$row = mysqli_fetch_array($type);
	$type_name = $row['type_name'];
	foreach ($test as $test1)
{
	$amenities_check = mysqli_query($connect, "INSERT INTO room_facility (room_id, facility) VALUES ('$text_id', '$test1')");
}
